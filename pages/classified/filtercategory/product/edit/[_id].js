...
const Edit = ({product}) => {
    ...
    const HandleSaveData = (e) => {
        e.preventDefault();
        axios.delete("http://localhost:8000/deleteproduct/"+product[0]._id,{
            _id:product[0]._id
        }).then(res=>{
        ...
        axios.post('http://localhost:8000/addproduct', fn)
            .then(res => {
                setBrand('')
                setPrice('')
                setSeller('')
    ...
    return (
        <div className="container">
            ...
            <span onClick={()=> router.push('/classified')}><a>&#8592;Back</a></span>
            <h1>Edit Product</h1>
            ...
            <div className="row">
                <div className="col-md-6">
                    <form className="form" onSubmit={HandleSaveData}>
                        <div className="form-group">
                            <label>Model</label>
                            <input type="text" value={model} className="form-control" onChange={(e) => setMode(e.target.value)} placeholder="Model" />
                        </div>
                        <div className="form-group">
                            <label>Category</label>
                            <input type="text" value={category} className="form-control" onChange={(e) => setCatgory(e.target.value)} placeholder="Category" />
                        </div>
                        <div className="form-group">
                            <label>Brand</label>
                            <input type="text" value={brand} className="form-control" onChange={(e) => setBrand(e.target.value)} placeholder="Brand" />
                        </div>
                        <div className="form-group">
                        ...
                        <div className="form-group">
                            <label>Seller</label>
                            <input type="text" value={seller} className="form-control" onChange={(e) => setSeller(e.target.value)} placeholder="Seller" />
                        </div>
                        <div className="form-group">
                            <label>Price</label>
                            <input type="number" value={price} className="form-control" onChange={(e) => setPrice(e.target.value)} placeholder="Price" />
                        </div>
                        <div className="form-group">
                            <label>Year</label>
                            <input type="text" value={year} className="form-control" onChange={(e) => SetYear(e.target.value)} placeholder="Year" />
                        </div>
                        <h5>Upload Image</h5>
                        <span>Min=1 Max=5</span>
                        <div >
                        <input type="file" name="image" onChange={fileChanges} required multiple />
                        </div>
                ...
                <div className="col-md-6">
                    ...
                    <div className="input-group mb-3">
                        <select className="custom-select" id="inputGroupSelect01" onChange={changeCategory}>
                    ...
                    <h5>Brand</h5>
                    <div className="input-group mb-3">
                        <select className="custom-select" id="inputGroupSelect01" onChange={changeBrand}>
                            <option selected>Choose...</option>
                            {uniqB.map(item => {
                                return (
                                    <option key={item}>{item}</option>
                    ...
                    <h5>Model</h5>
                    <div className="input-group mb-3">
                        <select className="custom-select" id="inputGroupSelect01" onChange={changeModel}>
                            <option selected>Choose...</option>
                            {uniqM.map(item => {
                                return (
                                    <option key={item}>{item}</option>
...
export async function getServerSideProps({ query }) {
    const { _id } = query;
    const res = await fetch("http://localhost:8000/getproduct/" + _id);
