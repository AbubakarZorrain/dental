...
export default function Category({ query }) {
  ...
  useEffect(() => {
    axios.get(`http://localhost:8000/getproduct`).then((res) => {
      const { data } = res;
      setDropdown(data);
    });
    axios
      .get(`http://localhost:8000/getproductcategory/${term}`)
      .then((res) => {
        const { data } = res;
        if (!data) {
    ...
    axios.get("http://localhost:8000/getrecentdata").then((res) => {
      const { data } = res;
      setRecentdata(data);
    });
  }, [term, category]);
            ...
            {recentdata.map((item) => {
              return (
                ...
                <div style={{ display: "flex", marginBottom: "5px", marginTop:"15px" }}>
                  <div style={{ width: "50%" }}>
                    <img style={{ minWidth:"90px", maxWidth:"90px", minHeight:"90px", maxHeight:"90px"}} src={item.productImage[0]} />
                  </div>
                  <div
...
const Carusel = ({ location, model, category, image1, price, year }) => {
  return (
    <div>
      <div
        style={{ padding: "0 5px 0 3px", maxWidth: "170px", margin: "15px 0px", position: "relative", border: "1px solid rgba(0, 0, 0, 0.1)"}}
        
      >
        <img style={{minWidth:"160px", maxWidth:"160px", minHeight:"180px", maxHeight:"180px"}} src={image1} />
        <p style={{marginLeft: "5px", marginBottom: "3px", marginTop:"5px", color: "Grey", fontWeight: "bold"}}>{model}</p>
